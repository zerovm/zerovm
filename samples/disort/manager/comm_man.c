...
recv_histograms( int fdr, struct Histogram *histograms, int wait_histograms_count ){
	...
	for( int i=0; i < wait_histograms_count; i++ ){
		...
		if ( EPACKET_HISTOGRAM == t.type ){
			...
			WRITE_FMT_LOG(LOG_ERR, "recv_histograms::wrong packet type %d size %d", t.type, (int)t.size);
			exit(-1);
		}
	}
...
reqrep_detailed_histograms_alloc( int fdw, int fdr, int nodeid,
	...
	char reply;
	WRITE_FMT_LOG(LOG_DEBUG, "[%d] complete=%d, Write detailed histogram requests to %d", nodeid, complete, fdw );
	int bytes;
