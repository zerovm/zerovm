 ...
 * It's code can be used unchanged for interprocess and tcp communications
 * because networking configuration is completely described by db_records data;
 * It's used by nodes as layer between file I/O and networking where write_sockf, read_sockf
 * main input output routines. Every communication socket (sock_file_t) is related
 * to own fd - file descriptor;
...
/*
 * open file socket is associated with fd; It's should be called close_sockf
 * for every opened file socket;
 *
...
struct sock_file_t* open_sockf
  (struct zeromq_pool* zpool, struct db_records_t *db_records, int fd);

/*
 * close file socket, opened by open_sockf; it's do not removing it
 * from sockets array only free zeromq resources;
 * @return err
 */
