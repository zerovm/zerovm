...
int capabilities_for_file_fd(int fd)
{
  if(__db_records)
  {
    ...
    if(db_record)
    {
      if('r' == db_record->fmode) return EREAD;
      else if('w' == db_record->fmode) return EWRITE;
    }
    else
    {
...
int init_zvm_networking(struct zmq_netw_interface *netw_if, const char *dbname,
    const char *nodename, int nodeid)
{
  uint64_t db_size = 0;
  int err = LOAD_OK;
  assert(nodename);
  ...
  for(int i = 0; i < __zpool->sockf_array->num_entries; i++)
  {
    ...
    if(sockf && ESOCKET_REQREP == sockf->sock_type)
    {
      if('r' == sockf->access_mode)
      {
        ...
        assert(
            sizeof(header) == __netw_if->write_sockf(sockf, (char*)&header, sizeof(header)));
